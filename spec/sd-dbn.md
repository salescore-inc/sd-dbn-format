...
### 1.3 責務外
本書は SD-DBN 文書のフォーマットだけを定義する。Agent、評価者、判定者、ルート割当、独立判定、
多数決、合意集約、評価レポート生成、アドバイス生成は本フォーマットの概念ではない。これらは
SD-DBN を生成・利用するアプリケーション、スキル、または運用プロセスの責務である。
必要な場合、外部プロセスは SD-DBN 文書とは別に Agent の判定ログや合意ログを保持し、その結果だけを
適合する `events` として追記する。
...
| **perspective** | 観点。到達状態判定の基準となる文脈(§6.4)。被評価対象とそれ以外を区別する。利用側が具体的役割を割り当てる。 |
| **evidence_state** | 到達状態。イベント化された変数または論じられた推論の到達の質。値は `agreed` / `mentioned` / `rejected`(§6.4)。 |
...
| `by_perspective` | | object | どの観点に由来するか(例 `{"<観点>": 1}`)。到達状態判定の根拠(§6.4) |
| `evidence_state` | | enum | 到達の質(§6.4) |
...
独立(§6.4.1)。推論への反論は新しい `argued` を追記して表す(append-only)。
...
| `carryover` | | object | 別の時間軸からの事前引き継ぎ(§6.5) |
...
| Application / inference layer | posterior 更新、欠損 CPD の補完、外部評価プロセスの結果反映を扱う |
